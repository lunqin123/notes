...
export type HudElement = 'project' | 'context' | 'usage' | 'promptCache' | 'cacheHitRate' | 'memory' | 'environment' | 'tools' | 'agents' | 'todos';
...
function validateCostCurrency(value: unknown): value is 'usd' | 'cny' | 'both' {
  return value === 'usd' || value === 'cny' || value === 'both';
}
...
export function mergeConfig(userConfig: Partial<HudConfig>): HudConfig {
  ...
  const display = {
    ...
    costCurrency: validateCostCurrency(migrated.display?.costCurrency)
      ? migrated.display.costCurrency
      : DEFAULT_CONFIG.display.costCurrency,
